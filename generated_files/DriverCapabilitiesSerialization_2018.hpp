...
namespace Materialise {
	const std::string schema_generated_files_DriverCapabilitiesSerialization_2018_namespace = "http://schemas.materialise.com/BuildProcessor/2018";
	...
	struct DriverPrivateCapabilities {
		void Write(IXmlSerializerWriter& s, const std::string& __name__);
		bool Read(IXmlSerializerReader& s, const std::string& __name__);
		DriverPrivateCapabilities(const DriverPrivateCapabilities&) = delete;
		DriverPrivateCapabilities& operator=(DriverPrivateCapabilities&&) = delete;
		DriverPrivateCapabilities& operator=(DriverPrivateCapabilities&) = delete;
		DriverPrivateCapabilities(DriverPrivateCapabilities&&) noexcept;
		DriverPrivateCapabilities() = default;
		~DriverPrivateCapabilities() = default;
...
Materialise::DriverCapabilities::DriverCapabilities(Materialise::DriverCapabilities &&___DriverCapabilities) noexcept
	...
	, NoMeshSupport(___DriverCapabilities.NoMeshSupport)
	, NoStandaloneSupport(___DriverCapabilities.NoStandaloneSupport)
	, SubcomponentProfileAssignment(___DriverCapabilities.SubcomponentProfileAssignment)
	, SupportsBeamLatticeCapModesButtAndSphere(___DriverCapabilities.SupportsBeamLatticeCapModesButtAndSphere)
...
bool Materialise::DriverCapabilities::Read(IXmlSerializerReader& s, const std::string& __name__) {
	...
	bool __NoMeshSupport;
	if (s.ReadBool("NoMeshSupport", __NoMeshSupport))
		NoMeshSupport = std::make_optional(__NoMeshSupport);
	bool __NoStandaloneSupport;
	if (s.ReadBool("NoStandaloneSupport", __NoStandaloneSupport))
		NoStandaloneSupport = std::make_optional(__NoStandaloneSupport);
	bool __SubcomponentProfileAssignment;
	if (s.ReadBool("SubcomponentProfileAssignment", __SubcomponentProfileAssignment))
		SubcomponentProfileAssignment = std::make_optional(__SubcomponentProfileAssignment);
	bool __SupportsBeamLatticeCapModesButtAndSphere;
	if (s.ReadBool("SupportsBeamLatticeCapModesButtAndSphere", __SupportsBeamLatticeCapModesButtAndSphere))
		SupportsBeamLatticeCapModesButtAndSphere = std::make_optional(__SupportsBeamLatticeCapModesButtAndSphere);
...
Materialise::DriverPrivateCapabilities::DriverPrivateCapabilities(Materialise::DriverPrivateCapabilities &&___DriverPrivateCapabilities) noexcept
	: NoSupportsAsSubcomponents(___DriverPrivateCapabilities.NoSupportsAsSubcomponents)
	, SupportsToolpathGeneration(___DriverPrivateCapabilities.SupportsToolpathGeneration)
	, DriverIssues(std::move(___DriverPrivateCapabilities.DriverIssues))
	, FileIO(std::move(___DriverPrivateCapabilities.FileIO))
	, NonGenericProfiles(___DriverPrivateCapabilities.NonGenericProfiles)
	, NoDefaultProfiles(___DriverPrivateCapabilities.NoDefaultProfiles)
	, NoMeshSupport(___DriverPrivateCapabilities.NoMeshSupport)
	, NoStandaloneSupport(___DriverPrivateCapabilities.NoStandaloneSupport)
	, SubcomponentProfileAssignment(___DriverPrivateCapabilities.SubcomponentProfileAssignment)
	, SupportsBeamLatticeCapModesButtAndSphere(___DriverPrivateCapabilities.SupportsBeamLatticeCapModesButtAndSphere)
	, Capabilities(std::move(___DriverPrivateCapabilities.Capabilities))
{ }
void Materialise::DriverPrivateCapabilities::Write(IXmlSerializerWriter& s, const std::string& __name__) {
	IXmlSerializerWriter::Scope scope(s, __name__);
...
bool Materialise::DriverPrivateCapabilities::Read(IXmlSerializerReader& s, const std::string& __name__) {
	IXmlSerializerReader::Scope scope(s, __name__);
	if (!scope.exist())
		return false;
	bool __NoSupportsAsSubcomponents;
	if (s.ReadBool("NoSupportsAsSubcomponents", __NoSupportsAsSubcomponents))
		NoSupportsAsSubcomponents = std::make_optional(__NoSupportsAsSubcomponents);
	bool __SupportsToolpathGeneration;
	if (s.ReadBool("SupportsToolpathGeneration", __SupportsToolpathGeneration))
		SupportsToolpathGeneration = std::make_optional(__SupportsToolpathGeneration);
	std::unique_ptr<Materialise::DriverIssues> __DriverIssues = std::make_unique<Materialise::DriverIssues>();
	if (__DriverIssues->Read(s, "DriverIssues"))
		DriverIssues = std::make_optional(std::move(__DriverIssues));
	std::unique_ptr<Materialise::FileIO> __FileIO = std::make_unique<Materialise::FileIO>();
