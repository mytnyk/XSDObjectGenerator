...
namespace Materialise {
	const std::string schema_generated_files_DeviceState_namespace = "urn:Print3D";
...
Materialise::DeviceStatus::DeviceStatus(Materialise::DeviceStatus &&___DeviceStatus) noexcept
	: value(___DeviceStatus.value)
	, description(___DeviceStatus.description)
{ }
...
bool Materialise::DeviceState::Read(IXmlSerializerReader& s, const std::string& __name__) {
	...
	std::unique_ptr<Materialise::Meters> __Meters = std::make_unique<Materialise::Meters>();
	if (__Meters->Read(s, "Meters"))
		Meters = std::make_optional(std::move(__Meters));
	std::unique_ptr<Materialise::OperationReport> __PrintOperationReport = std::make_unique<Materialise::OperationReport>();
	if (__PrintOperationReport->Read(s, "PrintOperationReport"))
		PrintOperationReport = std::make_optional(std::move(__PrintOperationReport));
	std::unique_ptr<Materialise::Log> __Log = std::make_unique<Materialise::Log>();
	if (__Log->Read(s, "Log"))
		Log = std::make_optional(std::move(__Log));
	return true;
