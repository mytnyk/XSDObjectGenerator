...
namespace Materialise {
	const std::string schema_generated_files_Documentation_namespace = "";
	struct Links;
	struct Link;
...
bool Materialise::Documentation::Read(IXmlSerializerReader& s, const std::string& __name__) {
	...
	if (!scope.exist())
		return false;
	std::unique_ptr<Materialise::Links> __Links = std::make_unique<Materialise::Links>();
	if (__Links->Read(s, "Links"))
		Links = std::make_optional(std::move(__Links));
	return true;
}
