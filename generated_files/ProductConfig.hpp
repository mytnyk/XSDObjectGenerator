...
namespace Materialise {
	const std::string schema_generated_files_ProductConfig_namespace = "";
	struct Product;
	struct Version;
...
Materialise::Version::Version(Materialise::Version &&___Version) noexcept
	: Major(___Version.Major)
	, Minor(___Version.Minor)
	, Build(___Version.Build)
	, Revision(___Version.Revision)
{ }
...
bool Materialise::Installer::Read(IXmlSerializerReader& s, const std::string& __name__) {
	...
	s.ReadStr("Name", Name);
	std::unique_ptr<Materialise::Filters> __Filters = std::make_unique<Materialise::Filters>();
	if (__Filters->Read(s, "Filters"))
		Filters = std::make_optional(std::move(__Filters));
	std::unique_ptr<Materialise::LicenseFiles> __LicenseFiles = std::make_unique<Materialise::LicenseFiles>();
	if (__LicenseFiles->Read(s, "LicenseFiles"))
		LicenseFiles = std::make_optional(std::move(__LicenseFiles));
...
bool Materialise::Filters::Read(IXmlSerializerReader& s, const std::string& __name__) {
	...
	if (!scope.exist())
		return false;
	std::unique_ptr<Materialise::ExcludeFiles> __ExcludeFiles = std::make_unique<Materialise::ExcludeFiles>();
	if (__ExcludeFiles->Read(s, "ExcludeFiles"))
		ExcludeFiles = std::make_optional(std::move(__ExcludeFiles));
	std::string __ExcludeFolders;
	if (s.ReadStr("ExcludeFolders", __ExcludeFolders))
		ExcludeFolders = std::make_optional(std::move(__ExcludeFolders));
	std::string __IncludeFolders;
	if (s.ReadStr("IncludeFolders", __IncludeFolders))
		IncludeFolders = std::make_optional(std::move(__IncludeFolders));
	return true;
}
Materialise::ExcludeFiles::ExcludeFiles(Materialise::ExcludeFiles &&___ExcludeFiles) noexcept
	: except(___ExcludeFiles.except)
{ }
...
bool Materialise::LicenseFiles::Read(IXmlSerializerReader& s, const std::string& __name__) {
	...
	if (!scope.exist())
		return false;
	std::string __Prerelease;
	if (s.ReadStr("Prerelease", __Prerelease))
		Prerelease = std::make_optional(std::move(__Prerelease));
	std::string __Release;
	if (s.ReadStr("Release", __Release))
		Release = std::make_optional(std::move(__Release));
	return true;
}
