...
void Materialise::DeviceStatus::Write(IXmlSerializerWriter& s) {
	IXmlSerializerWriter::Scope scope(s, "DeviceStatus");
...
bool Materialise::DeviceStatus::Read(IXmlSerializerReader& s) {
	IXmlSerializerReader::Scope scope(s, "DeviceStatus");
	if (scope.exist() == false)
		return false;
...
void Materialise::DeviceState::Write(IXmlSerializerWriter& s) {
	IXmlSerializerWriter::Scope scope(s, "DeviceState");
	Status.Write(s);
	Meters.value().get().Write(s);
	PrintOperationReport.value().get().Write(s);
	Log.value().get().Write(s);
}
bool Materialise::DeviceState::Read(IXmlSerializerReader& s) {
	IXmlSerializerReader::Scope scope(s, "DeviceState");
	if (scope.exist() == false)
		return false;
	Status.Read(s);
