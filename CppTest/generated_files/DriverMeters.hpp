...
void Materialise::DriverMeters::Write(IXmlSerializerWriter& s) {
	IXmlSerializerWriter::Scope scope(s, "DriverMeters");
	Meters.Write(s);
}
bool Materialise::DriverMeters::Read(IXmlSerializerReader& s) {
	IXmlSerializerReader::Scope scope(s, "DriverMeters");
	if (scope.exist() == false)
		return false;
	Meters.Read(s);
	return true;
