...
namespace Materialise {
	const std::string schema_generated_files3_MtlsBuildTicket_2017_namespace = "http://schemas.materialise.com/build_processing/2017/02";
...
bool Materialise::CT_BuildTicket::Read(IXmlSerializerReader& s, std::string __name__) {
	...
	if (scope.exist() == false)
		return false;
	bool* __has_property_template = new bool();
	if (s.ReadAttrBool("has_property_template", *__has_property_template))
		has_property_template = std::optional<std::reference_wrapper<bool>> { *__has_property_template };
	Info.Read(s, "Info");
...
void Materialise::Info::Write(IXmlSerializerWriter& s, std::string __name__) {
	IXmlSerializerWriter::Scope scope(s, __name__);
	Source.Write(s, "Source");
}
bool Materialise::Info::Read(IXmlSerializerReader& s, std::string __name__) {
	IXmlSerializerReader::Scope scope(s, __name__);
	if (scope.exist() == false)
		return false;
...
void Materialise::Source::Write(IXmlSerializerWriter& s, std::string __name__) {
	IXmlSerializerWriter::Scope scope(s, __name__);
	if (Id.has_value())
...
bool Materialise::Source::Read(IXmlSerializerReader& s, std::string __name__) {
	IXmlSerializerReader::Scope scope(s, __name__);
	if (scope.exist() == false)
		return false;
	std::string* __Id = new std::string();
	if (s.ReadAttrStr("Id", *__Id))
		Id = std::optional<std::reference_wrapper<std::string>> { *__Id };
	bool* __PropertyTemplate = new bool();
	if (s.ReadAttrBool("PropertyTemplate", *__PropertyTemplate))
		PropertyTemplate = std::optional<std::reference_wrapper<bool>> { *__PropertyTemplate };
	return true;
}
